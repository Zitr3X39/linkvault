(() => {
  ...
  const PLAY_SVG = '<svg viewBox="0 0 24 24" width="26" height="26" aria-hidden="true"><path d="M8 5v14l11-7z" fill="currentColor"/></svg>';
  ...
  function ytId(u) {
    try {
      ...
      if (/(^|\.)youtube\.com$/.test(h)) return url.searchParams.get("v") || (url.pathname.match(/\/(shorts|embed|live)\/([\w-]+)/) || [])[2] || "";
    } catch (e) {}
    return "";
  }
